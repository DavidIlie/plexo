...
- **User scoping**: `PLEX_TOKEN` determines which user's watch data (the token owner). `TAUTULLI_USER_ID` optionally filters Tautulli stats to one user (injected after call params into every `tautulliFetch`, so callers cannot override it). `VIEWER_DISPLAY` controls public activity identity (`hidden` default, `avatar`, `name`, `avatar-name`); public history DTOs strip raw Tautulli usernames in `src/server/cache/history.ts`.
...
- **Invalidation**: `POST /api/refresh` → `revalidateTag` per domain root (or `?scope=<root>`; `?hard=1` for immediate expiry). `GET` reports the active handler + roots + rate limits. Soft mode (default) is true stale-while-revalidate: handlers keep serving the old entry with `revalidate: -1` (background refresh) instead of nuking the cache; `?hard=1` expires immediately.
- **Cache scope caveat**: Redis durability covers `'use cache'` entries only. Per-URL page output (upgraded `/music/[ratingKey]` shells, revalidated route entries) goes through the legacy FileSystemCache in the container's ephemeral `.next` — lost on restart, re-materialized on demand (fast, since the underlying data is in Redis). Fine single-replica; multi-replica would need a legacy `cacheHandler` (singular) or a volume at `.next/server`.
- **Static assets**: `/_next/static` is served `public, max-age=31536000, immutable` by `next start`; `supportsImmutableAssets` is adapter-only, N/A for standalone Docker. A reverse proxy must pass through Cache-Control, not override it.
